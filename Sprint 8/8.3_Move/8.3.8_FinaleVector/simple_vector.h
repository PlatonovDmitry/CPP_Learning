...
public:
    ...
    explicit SimpleVector(size_t size)
    {
        ...
        std::fill(temp.Get(), &temp[size], Type{});
    ...
    {
        ArrayPtr<Type> temp(size.Capacity);
        std::fill(temp.Get(), &temp[size.Capacity], Type{});
        internal_array_.swap(temp);
    ...
    Iterator Insert(ConstIterator pos, Type&& value)
    {
        ...
        if (size_ == capacity_) {
            ...
            std::copy(
                std::make_move_iterator(internal_array_.Get()), 
                std::make_move_iterator(Iterator(pos)),
                temp.Get());
            output = std::copy_backward(
                std::make_move_iterator(Iterator(pos)), 
                std::make_move_iterator(&internal_array_[size_]), 
                &temp[size_ + 1]);
            internal_array_.swap(temp);
