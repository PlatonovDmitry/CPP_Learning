...
void Operate() {
    ...
    auto app_rnd_start = steady_clock::now();

    // заполним вектор случайными числами 0 и 1
    AppendRandom(random_bits, N);

    auto app_rnd_end = steady_clock::now();
    cerr << "Append random: "s << chrono::duration_cast<chrono::microseconds>(app_rnd_end - app_rnd_start).count() << " ms"s << endl;

    // перевернём вектор задом наперёд
    vector<int> reversed_bits = ReverseVector(random_bits);

    auto reverse_end = steady_clock::now();
    cerr << "Reverse: "s << chrono::duration_cast<chrono::microseconds>(reverse_end - app_rnd_end).count() << " ms"s << endl;
    ...
    auto counting_end = steady_clock::now();
    cerr << "Counting: "s << chrono::duration_cast<chrono::microseconds>(counting_end - reverse_end).count() << " ms"s << endl;
}
