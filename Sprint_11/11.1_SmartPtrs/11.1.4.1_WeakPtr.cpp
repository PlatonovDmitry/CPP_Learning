...
public:
    ...
    std::shared_ptr<Value> GetValue(const Key& key) {
        if(cache_.count(key)){
            auto result = cache_.at(key).lock();
            if(result != nullptr){
                return result;
            }
        }

        auto result = value_factory_(key);
        cache_[key] =  std::weak_ptr<Value> {result};
        return result;
    }
private:
...
public:
    ...
    explicit BookLoader(const BookStore& store, size_t& load_count) :
        store_(&store),
        counter_ptr_(&load_count)
    {

    }

    std::shared_ptr<Book> operator()(const std::string& title) const {
        auto content = store_->at(title);
        auto output = std::make_shared<Book>(title, content);
        ++ *counter_ptr_;
        return output;
    }
