...
void DrawAnimals(const std::vector<const Animal*>& animals, ostream& out) {
    for(const Animal* cur_animal : animals){
        if(const Drawable* drawable_animal = dynamic_cast<const Drawable*>(cur_animal)){
            drawable_animal->Draw(out);
        }
    }
}
...
void TalkToAnimals(const std::vector<const Animal*> animals, ostream& out) {
    for(const Animal* cur_animal : animals){
        if(const Speakable* drawable_animal = dynamic_cast<const Speakable*>(cur_animal)){
            drawable_animal->Speak(out);
        }
    }
}
