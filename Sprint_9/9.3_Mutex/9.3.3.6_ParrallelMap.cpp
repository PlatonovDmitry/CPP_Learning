...
public:
    ...
    maps_(vector<map<Key, Value>>(bucket_count)),
    ...
    Access operator[](const Key& key){
        return {lock_guard{locks_[key % bucket_count_]}, maps_[key % bucket_count_][key]};
    }

    std::map<Key, Value> BuildOrdinaryMap(){
        map<Key, Value> output;
        for(size_t i=0; i<bucket_count_; ++i){
            auto guard = lock_guard{locks_[i]};
            for(const auto& cur_pair : maps_[i]){
                output[cur_pair.first] = cur_pair.second;
            }
        }
        return output;
    }
