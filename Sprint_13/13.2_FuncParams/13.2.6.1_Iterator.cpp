...
template <typename ForwardOneIt, typename ForwardTwoIt>
void Merge(
    ForwardOneIt left_begin,
    ForwardOneIt left_end, 
    ForwardTwoIt right_begin, 
    ForwardTwoIt right_end, 
    ostream& out) 
{
    while (left_begin != left_end && right_begin != right_end) {
        if (less<>()(*left_begin, *right_begin)) {
            out << *left_begin++ << endl;
        } else {
            out << *right_begin++ << endl;
        }
    }
...
template <typename Conteiner1, typename Conteiner2>
void MergeSomething(const Conteiner1& src1, const Conteiner2& src2, ostream& out) {
    Merge(
        src1.begin(), 
        src1.end(), 
        src2.begin(), 
