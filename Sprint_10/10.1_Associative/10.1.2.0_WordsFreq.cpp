...
vector<pair<string, int>> GetSortedWordCounts(vector<string> words) {
    ...
    vector<pair<string, int>> counts_vector;
    copy_if(
        move_iterator(counts_map.begin()),
        move_iterator(counts_map.end()),
        back_inserter(counts_vector),
        [](const pair<string, int>& pair){return pair.first.size()==5;}
    );
