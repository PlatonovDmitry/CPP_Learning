...
public:
    ...
    int64_t GetCurrentBill(const VehiclePlate& car) const {
        auto total_time = complete_parks_.count(car) == 0 
            ? 0 
            :  duration_cast<seconds>(complete_parks_.at(car)).count();
        
        if(now_parked_.count(car) > 0){
            total_time += duration_cast<seconds>(Clock::now() - now_parked_.at(car)).count();
        }

        return total_time * cost_per_second_;
    }
    ...
    unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> EndPeriodAndGetBills() {
        vector<VehiclePlate> parked_cars;
        transform(
            now_parked_.begin(),
            now_parked_.end(),
            back_inserter(parked_cars),
            [](const pair<VehiclePlate, TimePoint>& pair){ return pair.first;}
        );

        for(const auto& cur_car : parked_cars){
            Withdraw(cur_car);
            Park(cur_car);
        }
        ...
        for_each(
            ...
            complete_parks_.end(),
            [&output, this](const pair<VehiclePlate, Duration>& pair){
                output[pair.first] = duration_cast<seconds>(pair.second).count() * cost_per_second_;
            }
        );
